...
The two clean data sets are ready to merge but since the they are too large to merge in the same session, we will save the cleaned data first and merge in a separate session. 
The plan is 

```{r}
apc_stops_final <- apc_stops_clean %>% 
  left_join(trip_changes_clean, by = "trip_id")

ggplot(apc_stops_final, aes(x = date)) +
  geom_histogram(binwidth = 1)

```

`apc_stops_clean %>% left_join(trip_changes_clean, by = "trip_id")`

and `mutate` a new column indicating whether the `date` is within `change_start_date` and `change_end_date`
