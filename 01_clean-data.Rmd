...
- `use_after_six`: Binary indicator for whether they reloaded the card 6+ months after card is issued
...
- `reenroll_after_study`: Binary indicator for whether the person reenrolled speciifcally after being assigned to one of the pseduo RCT treatment groups and receiving a passport previously

Treatment:

- Same as Q2-A and Q2-B, and maybe...
- `incentive_passport_binary`: Binary indicator for whether the person was in Phase 1 or Phase 2 of the pseduo RCT passport program
...
sales_before_after_six_months <- sales %>%
  ...
  mutate(use_after_six = total_amount_after_six > 0)
...
riders_final <- riders_clean %>% 
  ...
  mutate(incentive_cat = factor(`Initial Load`, levels = incentives, ordered = TRUE),
         incentive_passport_binary = !is.na(`Study Card`)) %>%
  group_by(id) %>% 
  mutate(across(c(incentive_cat, incentive_passport_binary), ~lag(.), .names = "{.col}_prev")) %>% 
  #
  ...
  mutate(ever_reenroll = any(times > 1),
         ...
         reenroll_after_study = incentive_passport_binary_prev & ever_reenroll) %>% 
  ungroup() %>% 
  #
  ...
  mutate(across(c(starts_with("total_amount"), starts_with("total_loadings"),
                ...
                ~replace_na(., 0))) %>% 
  replace_na(list(use_after_six = FALSE, incentive_passport_binary_prev = FALSE,
                  incentive_cat_prev = "0"))
riders_final
